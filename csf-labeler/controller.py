...
scanned_code = ""
input_code = ""
keys = "X^1234567890-XXXqwertzuiop{}XXasdfghjkl:'XXXyxcvbnm,.XXXX XXXXXXXXXXXXXXXX"
...
class CodeScanner(Thread):
    ...
    def run(self):
        global input_code, scanned_code, keys
...
class ButtonsReader(Thread):
    ...
    def send_print_helper(self, rounded_weight):
        global day_lot, count, unique_id
        count = count + 1                        
        label = generate_label(day_lot, count, str(rounded_weight), scanned_code)
        unique_id = label[:-4]
    ...
    def send_url_request(self):
        ...
        weight = current()
        url = "http://35.173.255.174/ws/weighted_package"
        request_data = {"receipt_number": unique_id,"packageId": scanned_code,"weight": weight}
